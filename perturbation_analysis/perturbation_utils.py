...
def check_near_collision(block, assembly, collision_thresh=.015):
    ...
    def _get_surface_distance(obj_id1, obj_id2):
        closest_points = p.getClosestPoints(obj_id1, obj_id2, distance=100)  # Get closest points with a large search radius
        if closest_points:
            return closest_points[0][8]  # The distance between the two objects (surface-to-surface)
        print("WARNING: NO CLOSEST POINT FOUND")
        return None

    for other_block in assembly.structure.structure:
        if other_block.id == block.id: # or block.id == 0 or other_block.id == 0: # I don't think structure would contain the ground plane ever?
            continue
        if _get_z_overlap(block, other_block) and _get_surface_distance(block.id, other_block.id) < collision_thresh:
            return True
...
def needs_perturbation(block, assembly):
    def _near_instability(block, assembly, min_radius=1, max_radius=4, num_circles=2, num_points_per_circle=8):
        positions = []
        circle_radii = np.linspace(min_radius, max_radius, num_circles)
        original_coordinates = block.position   
        
        for radius in circle_radii:
            positions.extend(circle_sample(radius, num_points_per_circle, original_coordinates).tolist())
    ...
    return (not check_block_status(block, assembly)) or _near_instability(block, assembly)
