...
from bloxnet.structure.structure import _create_cone, _create_cuboid, _create_cylinder, create_block
import os
...
def reset_blocks(init_pos):
    for block_id in init_pos:
        ...
        set_block_position_and_orientation(block_id, initial_position, initial_orientation)
...
def check_body_collision(bodyID = -1):
    ...
    for point in contact_points:
        ...
        if (bodyID == -1 or bodyID == bodyA or bodyID == bodyB) and point[8] < -10*EPSILON:
            return True
...
def check_for_collapse(id, start_pos, start_ori, position_threshold=0.1, rotation_threshold=0.1):
...
def get_num_collapse(init_pos, position_threshold=0.1, rotation_threshold=0.1):
    ...
    for id in init_pos:
        ...
        if check_for_collapse(id, start_pos, start_ori, position_threshold, rotation_threshold):
            num_collapses += 1
...
class Camera:
    ...
    def take_picture(self):
        ...
        width, height, rgbImg, depthImg, segImg = p.getCameraImage(
            width=500, height=500, viewMatrix=self.view_matrix,
            projectionMatrix=self.projection_matrix, renderer=p.ER_BULLET_HARDWARE_OPENGL
        )
        return rgbImg