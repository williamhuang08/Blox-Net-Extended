...
def add_axes_to_image(
    ...
    ax.set_xticks([i * tick_size for i in range(int(x_min / tick_size), int(x_max / tick_size) + 1)])
    ax.set_yticks([i * tick_size for i in range(int(y_min / tick_size), int(y_max / tick_size) + 1)])
    ...
    # Draw axis arrows
    arrowprops = dict(facecolor='black', edgecolor='black', arrowstyle='->', lw=1.0, mutation_scale=20)
...
def add_label(img, label, x, y, size=1.0):
    ...
    text_y = y
    cv2.putText(bgr_array, text, (text_x, text_y), font, font_scale, font_color, thickness, cv2.LINE_AA)
    rgb_array = cv2.cvtColor(bgr_array, cv2.COLOR_BGR2RGB)

    return rgb_array

def add_labels(img, view_matrix, projection_matrix, label_visually=True, label_size=1.0):
    ...
    try:
        ...
        raise ValueError("No ID == 0 found in pybullet simulation, this should be the plane")

    img_width, img_height = img.shape[1], img.shape[0]

    # get segmentation mask
    _, _, _, _, mask = p.getCameraImage(width=img_width,
                                        height=img_height,
...
def highlight_blocks(highlight_id, highlight_color = [1.0, 0.0, 0.0, 1.0], nonhighlight_color = [1.0, 1.0, 1.0, 1.0]):
    # Get the number of bodies in the simulation
    num_bodies = p.getNumBodies()
    # Retrieve the body IDs
    body_ids = [p.getBodyUniqueId(i) for i in range(num_bodies)]
...
def get_imgs(labels=False, axes=False, keys=['x', 'y', 'z', 'isometric'], size=0.3, width=640, height=640, highlight_id = None):
    ...
    for key in keys:
        ...
        view_matrix = p.computeViewMatrix(cameraEyePosition=eye,
        ...
        _, _, rgbImg, _, _ = p.getCameraImage(width=width,
                                                height=height,
                                                viewMatrix=view_matrix,
                                                projectionMatrix=projection_matrix)
        ...
        if labels:
            rgbImg = add_labels(rgbImg, view_matrix, projection_matrix, label_size=cam_info['label_size'])
...
if __name__ == "__main__":
    import matplotlib.pyplot as plt
    from bloxnet.pybullet.place_blocks_in_json import create_cuboid, init_pybullet, create_cylinder
    ...
    _create_cone(Block(999, 'cone', 'cone', 'cone',  [25, 50], [-75, 0, 25], [0,0,0,1], [0.5,0.5,0.5,1]))

    x,y,z,iso = imgs = get_imgs(labels=True, axes=True, size=.18)
