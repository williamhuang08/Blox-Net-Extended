from bloxnet.pipelines.whole_structure import get_structure_info, get_structure_rating, SAVE_DIR
import os
...
def _get_ratings(to_build, isometric_img, iter=0):
    ...
    response, eval_context = prompt_with_caching(prompt, [], structure_dir, "info_eval", cache=True, i=iter)
    ...
    response, eval_context = prompt_with_caching(prompt, eval_context, structure_dir, "rating_eval", cache=True, i=iter)
...
def get_ratings(assembly: Assembly):
    ...
    """
    guesses, rating = _get_ratings(assembly.to_build, assembly.isometric_image, iter=assembly.assembly_num)
