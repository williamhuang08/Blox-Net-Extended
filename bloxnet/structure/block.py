...
class Block:
    def __init__(self, id: int, block_name: str, gpt_name: str, shape: str, dimensions: list, position: list, orientation: list, color: list = [1.0, 0, 0, 1.0]) -> None:
        assert len(color) == 4, "color is not length 4"
        assert shape == "cuboid" or shape == "cylinder" or shape=="cone", f"shape must be cuboid or cylinder or cone: {shape}"
        if shape == "cuboid":
            assert len(dimensions) == 3, f"dimensions for cuboid not length 3: {dimensions}"
        elif shape == "cylinder" or shape == "cone":
            assert len(dimensions) == 2, f"dimensions for cylinder not length 2: {dimensions}"
        assert len(orientation) == 4 or len(orientation) == 3, f"orientation must be length 4 quaternion or length 3 euler angles: {orientation}"
    ...
    def get_json(self):
        dimensions = {
            ...
            "y": self.dimensions[1],
            "z": self.dimensions[2]
        } if self.shape == "cuboid" else {"radius": self.dimensions[0], "height": self.dimensions[1]}
