...
class Assembly:
    ...
    pre_perturbation_image: np.ndarray | None = None # isometric image before perturbation if perturbation is performed.

    def save_to_structure_dir(self, name = None):
        if name is None:
            self.save(f"{self.structure_directory}/assembly_obj_{self.assembly_num}.pkl")
        else:
    ...
    def load_all_assemblies_from_structure_dir(cls, structure_dir):
        pkl_files = [f for f in os.listdir(structure_dir) if f.endswith("pkl") and f.startswith("assembly_obj_")]
        return [
            cls.load_assembly_from_structure_dir(structure_dir, i) for i in range(len(pkl_files))
        ]
    ...
    def human_friendly_save(self, save_dir):
        ...
        if self.pre_perturbation_image is not None:
            self.pre_perturbation_image.save(os.path.join(save_dir, "pre_perturbation_image.png"))
