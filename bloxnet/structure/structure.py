...
from bloxnet.pybullet.place_blocks_in_json import move_until_contact, move_out_of_contact
import time
...
class Structure:
    ...
    def check_stability(self, id, place_all_blocks=False, drop=False, position_threshold=.01, rotation_threshold=.1, debug=False):       
        ...
        time_step = 1. / 240.  # 240 Hz simulation step
...
def _create_cuboid(block, lateral_friction = 0.5, spinning_friction = 0.2):
    dimensions, position, orientation, color = block.dimensions, block.position, block.orientation, block.color
    ...
    p.changeDynamics(id, -1, lateralFriction=lateral_friction, spinningFriction=spinning_friction)
    return id

def _create_cylinder(block, lateral_friction = 0.5, spinning_friction = 0.2):
    radius, height, position, orientation, color = block.dimensions[0], block.dimensions[1], block.position, block.orientation, block.color
    position_m = [x / 1000 for x in position]
...
def _create_cone(block, lateral_friction = 0.5, spinning_friction = 0.2):
    ...
    collision_shape = p.createCollisionShape(p.GEOM_MESH, fileName=CONE_STL_PATH, meshScale=[radius_m, radius_m, height_m])
    visual_shape = p.createVisualShape(p.GEOM_MESH, fileName=CONE_STL_PATH, meshScale=[radius_m, radius_m, height_m], rgbaColor=color)
    id = p.createMultiBody(baseMass=mass, baseCollisionShapeIndex=collision_shape, baseVisualShapeIndex=visual_shape, basePosition=position_m, baseOrientation=orientation)
    p.changeDynamics(id, -1, lateralFriction=lateral_friction, spinningFriction=spinning_friction)
    return id
...
def test_with_gui():
    ...
    structure = Structure()
    block1 = Block(id = 1, block_name = "block1", gpt_name = "box", shape = "cuboid", dimensions = [100, 100, 100], position = [0, 0, 50], orientation = [0, 0, 0, 1], color = [1.0, 0, 0, 1.0])
    block2 = Block(id = 2, block_name = "block2", gpt_name = "box", shape = "cuboid", dimensions = [180, 100, 180], position = [0, 0, 150], orientation = [0, 0, 0, 1], color = [1.0, 0, 0, 1.0])
    block3 = Block(id = 3, block_name = "block3", gpt_name = "box", shape = "cone", dimensions = [100, 500], position = [0, 0, 250], orientation = [0, 0, 0, 1], color = [1.0, 0, 0, 1.0])
